...
Runs a short 5-point calibration (look at each circle, press SPACE), then
opens a PsychoPy window with a gray background and draws a white circle over
wherever it thinks your eyes are looking, using MediaPipe's FaceLandmarker
...
def calibrate_point(camera, landmarker, win, dot, prompt, target_ratio) -> GazeFeatures | None:
...
def run_calibration(camera, landmarker, win, dot) -> dict[str, float] | None:
    ...
    try:
        for name, target_ratio in CALIBRATION_TARGETS.items():
            prompt.text = f"Look at the circle and press SPACE ({name})"
            result = None
            while result is None:
                result = calibrate_point(camera, landmarker, win, dot, prompt, target_ratio)
                if result is None:
...
def map_features_to_ratio(features: GazeFeatures, calibration: dict[str, float]) -> tuple[float, float]:
    ratio_x = calibration["x_slope"] * features.eye_x + calibration["x_intercept"]
    ratio_y = calibration["y_slope"] * features.eye_y + calibration["y_intercept"]
...
def main() -> None:
    ...
    try:
        calibration = run_calibration(camera, landmarker, win, dot)
        ...
        while True:
            ...
            if "r" in keys:
                ...
                new_calibration = run_calibration(camera, landmarker, win, dot)
