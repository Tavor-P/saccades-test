...
def _build_practice_schedule() -> list[bool]:
    ...
    return [i % 2 == 0 for i in range(NUM_PRACTICE_TRIALS)]
...
def build_saccade_sequence() -> list[TrialSpec]:
    ...
    practice_trials, source = _build_saccade_trials(_build_practice_schedule(), practice=True, start_source=Target.DOT)
    real_trials, _ = _build_saccade_trials(_build_shown_schedule(), practice=False, start_source=source)
